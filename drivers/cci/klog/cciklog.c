...
#ifdef CCI_HW_ID
#include <../drivers/misc/mediatek/cci_hw_id/cci_hw_id.h>
...
//hw_id
	...
	if(magic_update == 1 && device_info_update == 0)
	{
		...
		snprintf(klog_magic + KLOG_INFO_LENGTH + KLOG_IMAGE_INFO_LENGTH, KLOG_HW_ID_LENGTH, "%02X", get_cci_hw_id());
...
//recover hw_id
#ifdef CCI_HW_ID
						snprintf(sysinfo.hw_id, KLOG_HW_ID_LENGTH, "%02X", get_cci_hw_id());
#endif // #ifdef CCI_HW_ID
...
if(copy_from_user(&klog_type,  iov[3].iov_base, sizeof(klog_type)) == 0){
	 if(!strncmp(&klog_type, LOGGER_LOG_MAIN, sizeof(LOGGER_LOG_MAIN)))
	{
#if CCI_KLOG_ANDROID_MAIN_SIZE
	...
	else if(!strncmp(&klog_type, LOGGER_LOG_SYSTEM, sizeof(LOGGER_LOG_SYSTEM)))
	{
#if CCI_KLOG_ANDROID_SYSTEM_SIZE
		...
		else if(!strncmp(&klog_type, LOGGER_LOG_RADIO, sizeof(LOGGER_LOG_RADIO)))
	{
#if CCI_KLOG_ANDROID_RADIO_SIZE
	...
	else if(!strncmp(&klog_type, LOGGER_LOG_EVENTS, sizeof(LOGGER_LOG_EVENTS)))
	{
#if CCI_KLOG_ANDROID_EVENTS_SIZE
