...
static int mic_voltage1_comp; /* only for MIC mode = 6 */
int cur_key = 0;
...
char *accdet_report_string[4] = {
	...
	"Headset_mic",
	"Headset_no_mic",
	"HEADSET_illegal",  // BSP_Audio_S: B2359 20161229 FP019378:Non-CTIA headset warning message
	/* "Double_check"*/
};
/****************************************************************/
/***        export function                                                                        **/
...
static int Accdet_PMIC_IMM_GetOneChannelValue(int deCount)
{
	...
	vol_val -= mic_voltage1_comp;
	if (vol_val < 0)
		vol_val = 0;
	ACCDET_DEBUG("ACCDET offset: %d mv, vol: %d mv, comp: %d\n", accdet_auxadc_offset, vol_val, mic_voltage1_comp);
	return vol_val;
}
#endif
...
#define PIN_ADC_CHANNEL 5
...
#if 0  // BSP_Audio_S: B2359 20161229 FP019378:Non-CTIA headset warning message
						cable_type = HEADSET_NO_MIC;
						accdet_status = HOOK_SWITCH;
#else
						cable_type = HEADSET_ILEGAL;
						accdet_status = MIC_BIAS_ILLEGAL;
#endif  // BSP_Audio_E: 20161229
	...
	// BSP_Audio_S: B2359 20161229 FP019378:Non-CTIA headset warning message
	if (cable_type == HEADSET_ILEGAL ) {
		ACCDET_DEBUG("[Accdet]cable type:[%s], status switch:[%s]->[%s]\n",
		     accdet_report_string[3], accdet_status_string[pre_status],
...
static inline void accdet_init(void)
{
...
#else
	pmic_pwrap_write(ACCDET_DEBOUNCE1, 0x1000); // change to 1.0s
#endif // BSP_Audio_E: 20160130
	pmic_pwrap_write(ACCDET_DEBOUNCE3, cust_headset_settings->debounce3);
   ...
   /*********************ACCDET Analog Setting***********************************************************/
...
#ifndef ACCDET_WQHD
	pmic_set_register_value(PMIC_RG_AUDMICBIASVREF, ACCDET_MIC_VOL);
#else
	...
	if (accdet_dts_data.accdet_mic_mode == 1) {	/* ACC mode*/
		mic_voltage1_comp = 0;	/* The variation of the hook key */
		...
		mic_voltage1_comp = 20;	/* The variation of the hook key */
		pmic_pwrap_write(ACCDET_RSV, pmic_pwrap_read(ACCDET_RSV) | ACCDET_INPUT_MICP);
