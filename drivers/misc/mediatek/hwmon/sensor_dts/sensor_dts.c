...
struct alsps_hw *get_alsps_dts_func(const char *name, struct alsps_hw *hw)
{
	...
	ret = of_property_read_u32_array(node , "state_val", state_val,
		ARRAY_SIZE(state_val));
	if (ret == 0)
		hw->state_val		 = state_val[0];

	ret = of_property_read_u32_array(node , "psctrl_val", psctrl_val,
		ARRAY_SIZE(psctrl_val));
	if (ret == 0)
		hw->psctrl_val		 = psctrl_val[0];

	ret = of_property_read_u32_array(node , "alsctrl_val", alsctrl_val,
		ARRAY_SIZE(alsctrl_val));
	if (ret == 0)
		hw->alsctrl_val		 = alsctrl_val[0];

	ret = of_property_read_u32_array(node , "ledctrl_val", ledctrl_val,
		ARRAY_SIZE(ledctrl_val));
	if (ret == 0)
		hw->ledctrl_val		 = ledctrl_val[0];

	ret = of_property_read_u32_array(node , "wait_val", wait_val,
		ARRAY_SIZE(wait_val));
	if (ret == 0)
		hw->wait_val		 = wait_val[0];	
	} else {
