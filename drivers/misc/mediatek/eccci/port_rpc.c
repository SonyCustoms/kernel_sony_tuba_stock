...
int g_project_id = 0;
static int __init get_project_id_from_cmdline(char* cmdline)
{
	if(cmdline == NULL || strlen(cmdline) == 0)
	{
		CCCI_ERROR_LOG(0, RPC, "LE(K)=> projectid is empty\n");
	}
	else
	{
		g_project_id = simple_strtoul(cmdline, NULL, 16);
		CCCI_ERROR_LOG(0, RPC, "LE(K)=> projectid=%d\n", g_project_id);
	}

	return 0;
}
__setup("projectid=", get_project_id_from_cmdline);
...
static int get_eint_attr_val(int md_id, struct device_node *node, int index)
{
	...
	for (type = 0; type < SIM_HOT_PLUG_EINT_MAX; type++) {
		...
		if (type == SIM_HOT_PLUG_EINT_POLARITY) {
...
#if 1
		} else if( type ==  SIM_HOT_PLUG_EINT_SRCPIN ){
			...
			{
				value = 2;
				CCCI_ERROR_LOG(md_id, RPC,"[CEI] DS image, set eint source pin = %d, project_id=%d\n", value, project_id);
			}
			else // Single SIM
			{
				value = 1;
				CCCI_ERROR_LOG(md_id, RPC,"[CEI] SS image, set eint source pin = %d, project_id=%d \n", value, project_id);
			}
