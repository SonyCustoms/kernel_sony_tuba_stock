    ...
    case FLASH_IOC_GET_PRE_ON_TIME_MS_DUTY:
		{
			...
			if (pF != 0) {
				...
				logI("[FLASH_IOC_GET_PRE_ON_TIME_MS_DUTY] function pointer is wrong ~\n");
			}
			kdArg.arg = i4RetValue;
			if (copy_to_user
			    ((void __user *)arg, (void *)&kdArg, sizeof(kdStrobeDrvArg))) {
				logI("[FLASH_IOC_GET_PRE_ON_TIME_MS_DUTY] ioctl copy to user failed ~\n");
				return -EFAULT;
			}
			logI("FLASH_IOC_GET_PRE_ON_TIME_MS_DUTY line=%d duty=%d\n", __LINE__, i4RetValue);
		}
		break;
    ...
    case FLASH_IOC_GET_PRE_ON_TIME_MS:
		{		
			...
			if (pF != 0) {
				...
				logI("[FLASH_IOC_GET_PRE_ON_TIME_MS] function pointer is wrong ~\n");
			}
			kdArg.arg = i4RetValue;
			if (copy_to_user
			    ((void __user *)arg, (void *)&kdArg, sizeof(kdStrobeDrvArg))) {
				logI("[FLASH_IOC_GET_PRE_ON_TIME_MS] ioctl copy to user failed ~\n");
				return -EFAULT;
			}
			logI("FLASH_IOC_GET_PRE_ON_TIME_MS line=%d time_ms=%d\n", __LINE__, i4RetValue);
		}
	    break;
    //CEI comments end
    //CEI comments end
...
static ssize_t flashlight_flash_store(struct device *dev,
...
static DEVICE_ATTR(flash, 0644, flashlight_flash_show, flashlight_flash_store);
...
#define FLASHLIGHT_MAJOR 242
    ...
    ret = device_create_file(flashlight_device, &dev_attr_torch);
    if (ret) {
