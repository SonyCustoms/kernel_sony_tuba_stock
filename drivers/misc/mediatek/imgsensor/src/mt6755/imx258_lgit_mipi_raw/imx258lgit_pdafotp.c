...
#define PFX "IMX258_LGIT_pdafotp"
...
BYTE imx258lgit_eeprom_data[DATA_SIZE]= {0};
...
bool read_imx258lgit_eeprom( kal_uint16 addr, BYTE* data, kal_uint32 size){
	addr = 0x0763;
	size = 1404;
	...
	if(!get_done || last_size != size || last_offset != addr) {
		if(!_read_imx258_eeprom(addr, imx258lgit_eeprom_data, size)){
			get_done = 0;
            last_size = 0;
            last_offset = 0;
			return false;
	...
	memcpy(data, imx258lgit_eeprom_data, size);
    return true;
}

bool read_imx258lgit_eeprom_SPC( kal_uint16 addr, BYTE* data, kal_uint32 size){
