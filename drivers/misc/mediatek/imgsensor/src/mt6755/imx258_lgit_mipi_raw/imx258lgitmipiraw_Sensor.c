...
#include "imx258lgitmipiraw_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "IMX258_LGIT_camera_sensor"
#define LOG_1 LOG_INF("IMX258,MIPI 1LANE\n")
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = IMX258_LGIT_SENSOR_ID, //IMX258MIPI_SENSOR_ID,  /*sensor_id = 0x2680*/ //record sensor id defined in Kd_imgsensor.h
	...
	#if 0
	.cap2 = {/*PIP capture*/ //capture for PIP 24fps relative information, capture1 mode must use same framelength, linelength with Capture mode for shutter calculate
		.pclk = 415200000,
		.linelength = 5352,
		.framelength = 3228,
		.startx = 0,
...
kal_uint16 IMX258LGITMIPI_sensorGainMapping[IMX258MIPI_MaxGainIndex][2] ={
	{64 ,0	},
	{65 ,8	},
	{66 ,16 },
	{67 ,25 },
...
#define IMX258_LGIT_MIPI_MaxGainIndex2 (72)
kal_uint16 IMX258_LGIT_MIPI_sensorGainMapping2[IMX258_LGIT_MIPI_MaxGainIndex2][2] ={
	{72,0x0120 },
	{80,0x0140 },
	{88,0x0160 },
	{96,0x0180 },
...
extern bool read_imx258lgit_eeprom( kal_uint16 addr, BYTE* data, kal_uint32 size);
extern bool read_imx258lgit_eeprom_SPC( kal_uint16 addr, BYTE* data, kal_uint32 size);
...
static void load_imx258_SPC_Data(void)
{
	...
	if ( SPC_data_done == false ) {
		if (!read_imx258lgit_eeprom_SPC(0x0CDF,IMX258MIPI_SPC_Data,126)) {
	...
	if(write_cmos_sensor(0x0A02, 0x0F)==0){
		...
		while(retry--)
		{
			 if(read_cmos_sensor(0x0A01) == 0x01)
			 {
				 ...
				 LOG_INF("imx258 type = 0x%x(0x00=HDR,0x10=binning)", imx258_type);
				 return (kal_uint16)(((read_cmos_sensor(0x0A26) << 4) | (read_cmos_sensor(0x0A27) >> 4))+(IMX258_LGIT_SENSOR_ID - IMX258_SENSOR_ID));
			 }
		}
	}
...
static void set_shutter(unsigned long long shutter)
{
	...
	else
	{
	    ...
	    write_cmos_sensor(0x0104, 0x00);
	    LOG_INF("Exit! shutter =%lld, framelength =%d\n", shutter,imgsensor.frame_length);
	}
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
    ...
    for (i = 0; i < IMX258MIPI_MaxGainIndex; i++) {
        if(gain <= IMX258LGITMIPI_sensorGainMapping[i][0]){
            break;
        }
    }
    ...
    if(gain != IMX258LGITMIPI_sensorGainMapping[i][0])
            LOG_INF("Gain mapping don't correctly:%d %d \n", gain, IMX258LGITMIPI_sensorGainMapping[i][0]);
    return IMX258LGITMIPI_sensorGainMapping[i][1];
}
...
static kal_uint16 gain2reg2(const kal_uint16 gain)
{
    ...
    for (i = 0; i < IMX258_LGIT_MIPI_MaxGainIndex2; i++) {
        if(gain <= IMX258_LGIT_MIPI_sensorGainMapping2[i][0]){
            break;
        }
    }
	if(i==IMX258_LGIT_MIPI_MaxGainIndex2){
		i--;
	}
    if(gain != IMX258_LGIT_MIPI_sensorGainMapping2[i][0])
            LOG_INF("Gain mapping don't correctly:%d %d \n", gain, IMX258_LGIT_MIPI_sensorGainMapping2[i][0]);
    return IMX258_LGIT_MIPI_sensorGainMapping2[i][1];
}
...
static void PIP15fps_capture_setting()
{
	LOG_INF("PIP15fps capture E\n");
	write_cmos_sensor(0x0100,0x00);
	mdelay(10);
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if(imx258_type == IMX258_HDR_TYPE)
		sensor_info->PDAF_Support = 2; /*0: NO PDAF, 1: PDAF Raw Data mode, 2:PDAF VC mode(HDR), 3:PDAF VC mode(Binning)*/
	else
		sensor_info->PDAF_Support = 3; /*0: NO PDAF, 1: PDAF Raw Data mode, 2:PDAF VC mode(HDR), 3:PDAF VC mode(Binning)*/
...
//    unsigned long long *feature_return_para=(unsigned long long *) feature_para;
	...
	switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_PDAF_DATA:
			...
			read_imx258lgit_eeprom((kal_uint16 )(*feature_data),(char*)(uintptr_t)(*(feature_data+1)),(kal_uint32)(*(feature_data+2)));
			break;
		...
		case SENSOR_FEATURE_GET_VC_INFO:
            ...
            case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
				if(imx258_type == IMX258_HDR_TYPE)
					...
					memcpy((void *)pvcinfo,(void *)&SENSOR_VC_INFO_Binning[1],sizeof(SENSOR_VC_INFO_STRUCT));
                break;
            case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
				if(imx258_type == IMX258_HDR_TYPE)
                	memcpy((void *)pvcinfo,(void *)&SENSOR_VC_INFO[2],sizeof(SENSOR_VC_INFO_STRUCT));
				else
					memcpy((void *)pvcinfo,(void *)&SENSOR_VC_INFO_Binning[2],sizeof(SENSOR_VC_INFO_STRUCT));
                break;
            case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
            default:
				if(imx258_type == IMX258_HDR_TYPE)
					...
					memcpy((void *)pvcinfo,(void *)&SENSOR_VC_INFO_Binning[0],sizeof(SENSOR_VC_INFO_STRUCT));
                break;
            }
            break;
		/*PDAF CMD*/
