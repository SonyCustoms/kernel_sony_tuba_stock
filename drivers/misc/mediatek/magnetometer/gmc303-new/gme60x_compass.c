...
//jonny e-compass 2nd
#define GME60X_2ND 1
#define GPIO_GMC303_GP 5  //jonny gp low
...
struct mag_hw mag_cust_2;
static struct mag_hw *hw = &mag_cust_2;
...
static long AKECS_SetMode_SngMeasure(void)
{
	char buffer[2];
	/* Set measure mode */
	buffer[0] = GME60X_REG_CNTL2;
	buffer[1] = GME60X_MODE_SNG_MEASURE; 
...
static long AKECS_SetMode_SelfTest(void)
{
	...
	/* Set measure mode */
	buffer[0] = GME60X_REG_CNTL2;
	buffer[1] = GME60X_MODE_SELF_TEST;  
	/* Set data */
	return AKI2C_TxData(buffer, 2);
}
static long AKECS_SetMode_FUSEAccess(void)
{
	...
	/* Set measure mode */
	buffer[0] = GME60X_REG_CNTL2;
	buffer[1] = GME60X_MODE_FUSE_ACCESS;
	/* Set data */
	return AKI2C_TxData(buffer, 2);
}
static int AKECS_SetMode_PowerDown(void)
{
	...
	/* Set powerdown mode */
	buffer[0] = GME60X_REG_CNTL2;
	buffer[1] = GME60X_MODE_POWERDOWN;
	/* Set data */
	return AKI2C_TxData(buffer, 2);
}
...
static long AKECS_SetMode(char mode)
{
	...
	switch (mode & 0x1F){
			case GME60X_MODE_SNG_MEASURE:
			ret = AKECS_SetMode_SngMeasure();
			break;

			case GME60X_MODE_SELF_TEST:
			ret = AKECS_SetMode_SelfTest();
			break;

			case GME60X_MODE_FUSE_ACCESS:
			ret = AKECS_SetMode_FUSEAccess();
			break;

			case GME60X_MODE_POWERDOWN:
...
static int GME_CheckDevice(void)
{
	...
	databuf[0] = GME60X_REG_WIA1;   

	res = i2c_master_send(client, databuf, 0x1);
	if(res <= 0)
	{
		goto exit_GME303_CheckDeviceID;
	}
	...
	{
		res = -1;
		MAGN_ERR("GME303_CheckDeviceID 0x%x failt!\n ", databuf[0]);
		return res;
	}
	else
	{
		MAGN_LOG("GME303_CheckDeviceID 0x%x pass!\n ", databuf[0]);
	}

#if 1
	exit_GME303_CheckDeviceID:
	if (res <= 0)
	{
		MAGN_ERR("GME303_CheckDeviceID I2C failt!\n ");
		return  res;
	}
#endif
...
int FST_GME60X(void)
{
	...
	if (AKECS_SetMode(GME60X_MODE_FUSE_ACCESS) < 0) {
		printk(FST_TAG "%s:%d Error.\n", __func__, __LINE__);
		return 0;
	}
	...
	if (AKECS_SetMode(GME60X_MODE_POWERDOWN) < 0) {
		printk(FST_TAG "%s:%d Error.\n", __func__, __LINE__);
		return 0;
	}
	...
	if (AKECS_SetMode(GME60X_MODE_SNG_MEASURE) < 0) {
		printk(FST_TAG "%s:%d Error.\n", __func__, __LINE__);
		return 0;
	}
	...
	if (AKECS_SetMode(GME60X_MODE_SELF_TEST) < 0) {
		printk(FST_TAG "%s:%d Error.\n", __func__, __LINE__);
		return 0;
	}
...
static long gme60x_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
  ...
  //	MAG_ERR("gme60x cmd:0x%x\n", cmd);	
	...
	case ECS_IOCTL_GET_CONF_60X:
		...
		ret = AKECS_SetMode(GME60X_MODE_FUSE_ACCESS);
		if (ret < 0)
			return ret;

		sense_conf[0] = GME60X_FUSE_ASAX;
		...
		ret = AKECS_SetMode(GME60X_MODE_POWERDOWN);
		if (ret < 0)
			return ret;
...
/*----------------------------------------------------------------------------*/
int gme60x_operate(void *self, uint32_t command, void *buff_in, int size_in,
...
/*----------------------------------------------------------------------------*/
int gme60x_orientation_operate(void *self, uint32_t command, void *buff_in, int size_in,
	void *buff_out, int size_out, int *actualout)
{
	int err = 0;
	int value;
...
static int gme60x_m_enable(int en)
{
	...
	if (value == 1) {
		...
		err = AKECS_SetMode(GME60X_MODE_SNG_MEASURE);
		if (err < 0) {
		...
		if (atomic_read(&o_flag) == 0) {
			atomic_set(&open_flag, 0);
			err = AKECS_SetMode(GME60X_MODE_POWERDOWN);
			if (err < 0) {
				MAG_ERR("%s:AKECS_SetMode Error.\n", __func__);
				return err;
			}
...
static int gme60x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	//jonny m s
	MAGN_LOG("[%s] client->addr S= 0x%2X\n", __func__, client->addr);
	MAGN_LOG("[%s] i2c_addr[0] = 0x%2X\n", __func__, data->hw->i2c_addr[0]);
	//client->addr = 0;
	MAGN_LOG("[%s] client->addr E= 0x%2X\n", __func__, client->addr);
	//jonny m e
...
{
	const char *name = "mediatek,gme60x";
	//struct i2c_board_info gme60x_i2c = {0}; //JONNY add for compatible
...
//jonny test e
	...
	if( (g_iHwId == 0x0) || (g_iHwId == 0x01) || (g_iHwId == 0x02) || (g_iHwId == 0x03) || (g_iHwId == 0x04) || (g_iHwId == 0x05))
	{
		...
		MAGN_LOG("[%s] set PGIO output_low.. \n", __FUNCTION__);
		mt_set_gpio_mode(GPIO_GMC303_GP, 0);
		mt_set_gpio_dir(GPIO_GMC303_GP, 1);
		mt_set_gpio_out(GPIO_GMC303_GP, 0);
		udelay(100);
	}
	else{
		//PQ:0x6 or others
		// do nothing
		MAGN_LOG("[%s] hwid=PQ PGIO do nothing.. \n", __FUNCTION__);
	}
...
//jonny gp e
	...
	/*jonny add for compatible*/
#ifndef GME60X_2ND
	if (hw->i2c_addr[0] != 0){
		struct i2c_board_info gme60x_i2c={ I2C_BOARD_INFO(GME60X_DEV_NAME, hw->i2c_addr[0])};
		
		i2c_register_board_info(hw->i2c_num, &gme60x_i2c, 1);
		
		MAGN_LOG("[%s] gme60x_i2c.type = %s\n", __func__, gme60x_i2c.type);
		MAGN_LOG("[%s] gme60x_i2c.addr = %x\n", __func__, gme60x_i2c.addr);
	}
	/*end*/
#endif
