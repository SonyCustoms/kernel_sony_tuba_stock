...
#if 1   //henry add
atomic_t g_iIsSuspend;
int g_iAddTime = 200;   //unit is ms
int g_iSetFlagTime = 200;   //unit is ms; unused
struct timer_list g_sTimer;
int g_iSetFlagTime2 = 10;   //HZ/5 = 200ms, HZ/10 = 100ms
int g_iAddSkipMag = 1;
#endif
...
static void mag_work_func(struct work_struct *work)
{
	...
	ii = atomic_read(&g_iIsSuspend);
    	//Printhh("[%s] ii = %d \n", __FUNCTION__, ii);
        if(ii == 1){
            //Printhh("[%s] suspend flag is set, skip job.. \n", __FUNCTION__);
            if (true == cxt->is_polling_run){
                Printhh("[%s] skip job..., suspend flag is set and resume not ready!\n", __FUNCTION__);
                MAG_ERR("skip job..., suspend flag is set and resume not ready!\n");
                //Printhh("[%s] call  startTimer(%d)ms \n", __FUNCTION__, atomic_read(&cxt->delay));
                startTimer(&cxt->hrTimer, (atomic_read(&cxt->delay) + g_iAddTime), false); // == mag_poll()
                //startTimer(&cxt->hrTimer, atomic_read(&cxt->delay) , false); // == mag_poll()
                return;
            }
        }
    }
	...
	for (i = 0; i < MAX_M_V_SENSOR; i++) {
		if (!(cxt->active_data_sensor&(0x01<<i))) {
			//Printhh("[%s] mag_type(%d)  enabled(%d)\n", __FUNCTION__, i, cxt->active_data_sensor);
	...
	if (true == cxt->is_polling_run){
		//Printhh("[%s] is_polling_run==true, call startTimer(hrTimer) \n", __FUNCTION__);
		//Printhh("[%s] cxt->delay = %d ms \n", __FUNCTION__, atomic_read(&cxt->delay));
		startTimer(&cxt->hrTimer, atomic_read(&cxt->delay), false); // == mag_poll()
	    }
}
...
#if 0   //; unused
static void mag_work_func2(struct work_struct *work)
{
       Printhh("[%s] call  add_timer(%d) \n", __FUNCTION__, g_iSetFlagTime2);
       g_sTimer.expires = jiffies + (HZ/g_iSetFlagTime2);
       add_timer(&g_sTimer);    // == mag_set_resume_param3()
}
#endif
...
enum hrtimer_restart mag_poll(struct hrtimer *timer)
{
	struct mag_context *obj = (struct mag_context *)container_of(timer, struct mag_context, hrTimer);
    	...
    	//Printhh("[%s] call queue_work(, report) \n", __FUNCTION__);
	queue_work(obj->mag_workqueue, &obj->report); //==mag_work_func()

	return HRTIMER_NORESTART;
}


#if 1   //henry, workaround //; unused
enum hrtimer_restart mag_set_resume_param2(struct hrtimer *timer)
{
	//struct mag_context *obj = (struct mag_context *)container_of(timer, struct mag_context, hrTimerResume);
        
	atomic_set(&g_iIsSuspend, 0);
	Printhh("[%s] new g_iIsSuspend = %d \n", __FUNCTION__, atomic_read(&g_iIsSuspend));
...
#if 1   //henry, workaround 
static void mag_set_resume_param(unsigned long ulData)
{
...
static struct mag_context *mag_context_alloc_object(void)
{
	...
	atomic_set(&obj->wake, 0);
    	...
    	//Printhh("[%s] call INIT_WORK(report, mag_work_func) \n", __FUNCTION__);
	INIT_WORK(&obj->report, mag_work_func);
#if 0   //; unused
	INIT_WORK(&obj->setFlag, mag_work_func2);
#endif
	...
	if (!obj->mag_workqueue) {
    	...
    	//Printhh("[%s] call initTimer(hrTimer, mag_poll) \n", __FUNCTION__);
	initTimer(&obj->hrTimer, mag_poll);

#if 0 //; unused
	hrtimer_init(&obj->hrTimerResume2, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	obj->hrTimerResume2.function = mag_set_resume_param2;
#endif

#if 1
        init_timer(&g_sTimer);
        g_sTimer.function = &mag_set_resume_param;
        g_sTimer.data = (0);
        g_sTimer.expires = jiffies + HZ/10;
#endif
...
static int mag_enable_data(int handle, int enable)
{
	...
	if (1 == enable) {
		...
		if ((0 != cxt->active_data_sensor) && (false == cxt->is_polling_run) &&
			...
			if (false == cxt->mag_ctl.is_report_input_direct) {
				...
				//Printhh("[%s] is_polling_run==false, call startTimer(hrTimer) \n", __FUNCTION__);
	...
	if (0 == enable) {
		...
		if (ID_M_V_MAGNETIC == handle) {
			cxt->mag_ctl.m_enable(0);   //==yas_m_enable()
			cxt->mag_ctl.m_open_report_data(0); //==yas_m_open_report_data()
		}
		...
		if (0 == cxt->active_data_sensor && true == cxt->is_polling_run) {
			if (false == cxt->mag_ctl.is_report_input_direct) {
				...
				//Printhh("[%s] is_polling_run==true, call stopTimer(hrTimer) \n", __FUNCTION__);
...
#if 1   //henry tt
static ssize_t mag_show_resume_tm(struct device *dev,
				 struct device_attribute *attr, char *buf)
{

	Printhh("[%s] g_iAddTime = %d \n", __FUNCTION__, g_iAddTime);
	return snprintf(buf, PAGE_SIZE, "%d\n", g_iAddTime);
}

static ssize_t mag_store_resume_tm(struct device *dev, struct device_attribute *attr,
				  const char *buf, size_t count)
{
	int64_t iDelay = 0;
	int ret = 0;

	ret = kstrtoll(buf, 10, &iDelay);
	if (ret != 0) {
		MAG_ERR("invalid format!!\n");
...
static ssize_t mag_show_flag_tm(struct device *dev,
				 struct device_attribute *attr, char *buf)
{

	//Printhh("[%s] g_iSetFlagTime = %d \n", __FUNCTION__, g_iSetFlagTime);
	//return snprintf(buf, PAGE_SIZE, "%d\n", g_iSetFlagTime);
	Printhh("[%s] g_iSetFlagTime = %d \n", __FUNCTION__, g_iSetFlagTime2);
	return snprintf(buf, PAGE_SIZE, "%d\n", g_iSetFlagTime2);
}

static ssize_t mag_store_flag_tm(struct device *dev, struct device_attribute *attr,
				  const char *buf, size_t count)
{
	int64_t iDelay = 0;
	int ret = 0;
	...
	if (ret != 0) {
        ...
        //g_iSetFlagTime = iDelay;
	//Printhh("[%s] g_iSetFlagTime = %d \n", __FUNCTION__, g_iSetFlagTime);
        g_iSetFlagTime2 = iDelay;
	Printhh("[%s] g_iSetFlagTime = %d \n", __FUNCTION__, g_iSetFlagTime2);
	return count;
}
...
#if 1   //henry tt
static ssize_t mag_show_add_skip(struct device *dev,
				 struct device_attribute *attr, char *buf)
{

	Printhh("[%s] g_iAddSkipMag = %d \n", __FUNCTION__, g_iAddSkipMag);
	return snprintf(buf, PAGE_SIZE, "%d\n", g_iAddSkipMag);
}

static ssize_t mag_store_add_skip(struct device *dev, struct device_attribute *attr,
				  const char *buf, size_t count)
{
	int64_t iDelay = 0;
	int ret = 0;
...
#if 1  //henry add
static int hh_suspend(struct platform_device *dev, pm_message_t state)
{


    //Printhh("[%s] org g_iIsSuspend = %d \n", __FUNCTION__, atomic_read(&g_iIsSuspend));
    atomic_set(&g_iIsSuspend, 1);
    Printhh("[%s] new g_iIsSuspend = %d \n", __FUNCTION__, atomic_read(&g_iIsSuspend));

    return 0;
}

static int hh_resume(struct platform_device *dev)
{
    struct mag_context *cxt = NULL;


    cxt = mag_context_obj;
    Printhh("[%s] enter.. cxt->is_polling_run = %d \n", __FUNCTION__, cxt->is_polling_run);
    if(g_iAddSkipMag ==1)
    {

	//Printhh("[%s] enter.. cxt->is_polling_run = %d \n", __FUNCTION__, cxt->is_polling_run);
	if (true == cxt->is_polling_run) {
    	    Printhh("[%s] somethig wrong..\n", __FUNCTION__);

#if 1
           Printhh("[%s] call  add_timer(%d) \n", __FUNCTION__, g_iSetFlagTime2);
           g_sTimer.expires = jiffies + (HZ/g_iSetFlagTime2);
           add_timer(&g_sTimer);    // == mag_set_resume_param()
           return 0;
#endif

#if 0   //; unused
            Printhh("[%s] call  hrtimer_start(%d)ms \n", __FUNCTION__, g_iSetFlagTime);
            hrtimer_start(&cxt->hrTimerResume2, ktime_set(g_iSetFlagTime/1000, (g_iSetFlagTime % 1000) * 1000000),HRTIMER_MODE_REL);
            //==mag_set_resume_param2()
            return 0;
#endif
...
#if 0   //; unused
 	queue_work(cxt->mag_workqueue, &cxt->setFlag);//==mag_work_func2()
#endif
...
#if 1   //henry tt
DEVICE_ATTR(magresumetm,	 S_IWUSR | S_IRUGO, mag_show_resume_tm, mag_store_resume_tm);
DEVICE_ATTR(magflagtm,	 S_IWUSR | S_IRUGO, mag_show_flag_tm, mag_store_flag_tm);
DEVICE_ATTR(magaddskip,	 S_IWUSR | S_IRUGO, mag_show_add_skip, mag_store_add_skip);
#endif
...
#if 1   //henry tt
	&dev_attr_magresumetm.attr,
	&dev_attr_magflagtm.attr,
	&dev_attr_magaddskip.attr,
#endif
	NULL
};
...
static int check_abnormal_data(long x, long y, long z, int status)
{
	...
	if ((total < 100) || (total > 10000)) {
		if (count % 10 == 0){
			...
			Printhh("[%s] mag sensor abnormal data:\n", __FUNCTION__);// show on UART, use to debug.
		}
		count++;
		if (count > 1000)
			count = 0;
