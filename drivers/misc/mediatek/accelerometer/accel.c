...
#if 1   //henry add
atomic_t g_iIsSuspendAcc;
int g_iAddTimeAcc = 100;   //unit is ms
struct timer_list g_sTimerAcc;
int g_iSetFlagTimeAcc = 10;   //HZ/5 = 200ms, HZ/10 = 100ms
int g_iAddSkipAcc = 1;
#endif
...
static void acc_work_func(struct work_struct *work)
{
    ...
    if(g_iAddSkipAcc ==1)
    {
	ii = atomic_read(&g_iIsSuspendAcc);
        if(ii == 1){
            if (true == cxt->is_polling_run){
                #if 0
                Printhh("[%s] (Acc), cxt->is_polling_run is true!\n", __FUNCTION__);
                //Printhh("[%s] atomic_read(&cxt->delay) = %d\n", __FUNCTION__, atomic_read(&cxt->delay));  //10ms
                #endif
                
                #if 1
                Printhh("[%s] skip job(Acc)..., suspend flag is set and resume not ready!\n", __FUNCTION__);
                ACC_ERR("skip job(Acc)..., suspend flag is set and resume not ready!\n");
                startTimer(&cxt->hrTimer, (atomic_read(&cxt->delay) + g_iAddTimeAcc), false);
	...
	if (NULL == cxt->acc_data.get_data) {
		...
		Printhh("[%s] acc driver not register data path\n", __FUNCTION__);
		return;
	}

	cur_ns = getCurNS();

        //Printhh("[%s] call  cxt->acc_data.get_data()\n", __FUNCTION__);
	err = cxt->acc_data.get_data(&x, &y, &z, &status);  //==gsensor_get_data()

	if (err) {
		ACC_ERR("get acc data fails!!\n");
		Printhh("[%s] get acc data fails!!\n", __FUNCTION__);
		goto acc_loop;
	} else {
			if (0 == x && 0 == y && 0 == z){
				Printhh("[%s] x==0, y==0, z==0!\n", __FUNCTION__);
				goto acc_loop;
			}
...
enum hrtimer_restart acc_poll(struct hrtimer *timer)
{
	...
	queue_work(obj->accel_workqueue, &obj->report); // == acc_work_func()
...
static void acc_set_resume_param(unsigned long ulData)
{

        
	atomic_set(&g_iIsSuspendAcc, 0);
	Printhh("[%s] new g_iIsSuspendAcc = %d \n", __FUNCTION__, atomic_read(&g_iIsSuspendAcc));
...
#if 1
        init_timer(&g_sTimerAcc);
        g_sTimerAcc.function = &acc_set_resume_param;
        g_sTimerAcc.data = (0);
        g_sTimerAcc.expires = jiffies + HZ/10;
#endif
	...
	acc_real_enable(enable);
		if (false == cxt->is_polling_run && cxt->is_batch_enable == false) {
			if (false == cxt->acc_ctl.is_report_input_direct) {
				startTimer(&cxt->hrTimer, atomic_read(&cxt->delay), true);  //==acc_poll()
...
#if 1   //henry tt
static ssize_t acc_show_resume_tm(struct device *dev,
				 struct device_attribute *attr, char *buf)
{

	Printhh("[%s] g_iAddTimeAcc = %d \n", __FUNCTION__, g_iAddTimeAcc);
	return snprintf(buf, PAGE_SIZE, "%d\n", g_iAddTimeAcc);
}

static ssize_t acc_store_resume_tm(struct device *dev, struct device_attribute *attr,
				  const char *buf, size_t count)
{
	int64_t iDelay = 0;
	int ret = 0;
	...
	if (ret != 0) {
        ...
        g_iAddTimeAcc = iDelay;
	Printhh("[%s] g_iAddTimeAcc = %d \n", __FUNCTION__, g_iAddTimeAcc);
	return count;
}


static ssize_t acc_show_flag_tm(struct device *dev,
				 struct device_attribute *attr, char *buf)
{

	Printhh("[%s] g_iSetFlagTimeAcc = %d \n", __FUNCTION__, g_iSetFlagTimeAcc);
	return snprintf(buf, PAGE_SIZE, "%d\n", g_iSetFlagTimeAcc);
}

static ssize_t acc_store_flag_tm(struct device *dev, struct device_attribute *attr,
				  const char *buf, size_t count)
{
	int64_t iDelay = 0;
	int ret = 0;
...
#if 1   //henry tt
static ssize_t acc_show_add_skip(struct device *dev,
				 struct device_attribute *attr, char *buf)
{

	Printhh("[%s] g_iAddSkipAcc = %d \n", __FUNCTION__, g_iAddSkipAcc);
	return snprintf(buf, PAGE_SIZE, "%d\n", g_iAddSkipAcc);
}

static ssize_t acc_store_add_skip(struct device *dev, struct device_attribute *attr,
				  const char *buf, size_t count)
{
	int64_t iDelay = 0;
	int ret = 0;
...
#if 1  //henry add
static int hh_suspend_acc(struct platform_device *dev, pm_message_t state)
{

#if 1   //henry add
    //Printhh("[%s] org g_iIsSuspend = %d \n", __FUNCTION__, atomic_read(&g_iIsSuspend));
    atomic_set(&g_iIsSuspendAcc, 1);
    Printhh("[%s] new g_iIsSuspendAcc = %d \n", __FUNCTION__, atomic_read(&g_iIsSuspendAcc));
#endif
...
static int hh_resume_acc(struct platform_device *dev)
{
    struct acc_context *cxt = NULL;
	...
	if (true == cxt->is_polling_run) {
           ...
           g_sTimerAcc.expires = jiffies + (HZ/g_iSetFlagTimeAcc);
           add_timer(&g_sTimerAcc);    // == acc_set_resume_param()
           return 0;
#endif
...
#if 1 //henry add
	.suspend = hh_suspend_acc,
	.resume = hh_resume_acc,
#endif
	.driver = {
		   .name = "gsensor",
	#ifdef CONFIG_OF
...
#if 1   //henry tt
DEVICE_ATTR(accresumetm,	 S_IWUSR | S_IRUGO, acc_show_resume_tm, acc_store_resume_tm);
DEVICE_ATTR(accflagtm,	 S_IWUSR | S_IRUGO, acc_show_flag_tm, acc_store_flag_tm);
DEVICE_ATTR(accaddskip,	 S_IWUSR | S_IRUGO, acc_show_add_skip, acc_store_add_skip);
#endif
...
#if 1   //henry tt
	&dev_attr_accresumetm.attr,
	&dev_attr_accflagtm.attr,
	&dev_attr_accaddskip.attr,
#endif
	NULL
};
