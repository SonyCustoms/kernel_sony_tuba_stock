...
struct md_ctl_block_t {
	...
	struct MD_CALL_BACK_HEAD_T md_notifier;
	int md_boot_stage;
...
atomic_t data_connect_sta[MAX_MD_NUM];
			...
			unsigned int *len)
{
	...
	if (addr_phy != NULL) {
		*(unsigned int *)addr_phy =
		md_ctlb[md_id]->
		smem_table->ccci_mdlog_smem_base_phy;
	}
	if (addr_vir != NULL) {
		*(unsigned int *)addr_vir =
		md_ctlb[md_id]->
		smem_table->ccci_mdlog_smem_base_virt;
	}
	if (len != NULL)
		      ...
		      unsigned int *len)
{
	...
	if (addr_phy != NULL)
		*(unsigned int *)addr_phy =
		md_ctlb[md_id]->smem_table->ccci_pcm_smem_base_phy;
	if (addr_vir != NULL)
		*(unsigned int *)addr_vir =
		md_ctlb[md_id]->smem_table->ccci_pcm_smem_base_virt;
	if (len != NULL)
		       ...
		       unsigned int *len)
{
	...
	if (addr_vir != NULL)
		*(unsigned int *)addr_vir =
		    md_ctlb[md_id]->smem_table->ccci_uart_smem_base_virt[port];
	if (addr_phy != NULL)
		*(unsigned int *)addr_phy =
		md_ctlb[md_id]->smem_table->ccci_uart_smem_base_phy[port];
	if (len != NULL)
		...
		unsigned int *len)
{
	...
	if (addr_vir != NULL)
		*(unsigned int *)addr_vir =
		md_ctlb[md_id]->smem_table->ccci_fs_smem_base_virt;

	if (addr_phy != NULL)
		*(unsigned int *)addr_phy =
		md_ctlb[md_id]->smem_table->ccci_fs_smem_base_phy;
...
int ccci_rpc_base_req(int md_id, int *addr_vir, int *addr_phy, int *len)
{
	...
	if (addr_phy != NULL)
		*(unsigned int *)addr_phy =
		md_ctlb[md_id]->smem_table->ccci_rpc_smem_base_phy;
	if (len != NULL)
...
int ccci_pmic_base_req(int md_id, void *addr_vir, void *addr_phy, int *len)
{
	...
	if (addr_vir != NULL)
		*(unsigned int *)addr_vir =
		md_ctlb[md_id]->
		smem_table->ccci_pmic_smem_base_virt;
	if (addr_phy != NULL)
		*(unsigned int *)addr_phy =
		md_ctlb[md_id]->smem_table->ccci_pmic_smem_base_phy;
	if (len != NULL)
...
int ccci_ipc_base_req(int md_id, void *addr_vir, void *addr_phy, int *len)
{
	...
	if (addr_vir != NULL)
		*(unsigned int *)addr_vir =
			md_ctlb[md_id]->smem_table->ccci_ipc_smem_base_virt;
	if (addr_phy != NULL)
		*(unsigned int *)addr_phy =
			md_ctlb[md_id]->smem_table->ccci_ipc_smem_base_phy;
	if (len != NULL)
...
int ccmni_v2_ul_base_req(int md_id, void *addr_vir, void *addr_phy)
{
	...
	if (addr_vir != NULL)
		*(unsigned int *)addr_vir =
		md_ctlb[md_id]->
		smem_table->ccci_ccmni_smem_ul_base_virt;

	if (addr_phy != NULL)
		*(unsigned int *)addr_phy =
		md_ctlb[md_id]->
		smem_table->ccci_ccmni_smem_ul_base_phy;
...
int ccmni_v2_dl_base_req(int md_id, void *addr_vir, void *addr_phy)
{
	...
	if (addr_vir != NULL)
		*(unsigned int *)addr_vir =
		md_ctlb[md_id]->
		smem_table->ccci_ccmni_smem_dl_base_virt;

	if (addr_phy != NULL)
		*(unsigned int *)addr_phy =
		md_ctlb[md_id]->
		smem_table->ccci_ccmni_smem_dl_base_phy;
				   ...
				   int *addr_phy, int *len)
{
	...
	if (md_ctlb[md_id] == NULL)
		return -CCCI_ERR_FATAL_ERR;
	*addr_virt = md_ctlb[md_id]->
		    smem_table->ccci_ccmni_ctl_smem_base_virt[port];
	*addr_phy = md_ctlb[md_id]->
		    smem_table->ccci_ccmni_ctl_smem_base_phy[port];
...
/*                                                                          */
/****************************************************************************/
static int send_ccci_system_ch_msg(int md_id, unsigned int msg_id,
				   unsigned int data)
{
...
int ccci_dormancy(int md_id, char *buf, unsigned int len)
{
	if (atomic_read(&data_connect_sta[md_id]) > 0) {
		CCCI_MSG_INF(md_id, "ctl",
			     "notify enter dormancy, data_sta<%d>\n",
...
size_t ccci_ch_filter_store(char buf[], size_t len)
{
	...
	ret =
	    sscanf(buf, "-c=%d 0x%x 0x%llx 0x%llx 0x%x 0x%x 0x%x ", &md_id,
		   &key, &lg_ch_tx_mask, &lg_ch_rx_mask, &fs_tx_mask,
		   &fs_rx_mask, &tty_mask);
	if (ret != 7)
		...
		CCCI_MSG("%x %x %x %x %llx %llx\n", key, fs_tx_mask, fs_rx_mask,
			 tty_mask, lg_ch_tx_mask, lg_ch_rx_mask);
	...
	for (i = 0; i < md_num; i++)
		if (get_modem_is_enabled(i))
			curr +=
			    snprintf(&buf[curr], len - curr,
				     "[MD%d] tx_mask: %llX\nrx_mask: %llX\nfs_tx_mask: %X\nfs_rx_mask: %X\ntty_msk: %X\n",
				     i + 1, lg_ch_tx_debug_enable[i],
				     lg_ch_rx_debug_enable[i],
				     fs_tx_debug_enable[i],
				     fs_rx_debug_enable[i],
				     tty_debug_enable[i]);
	return curr;
}
...
void ccci_ee_info_dump(int md_id, struct DEBUG_INFO_T *debug_info)
{
	...
	CCCI_DBG_MSG(md_id, "cci", "Dump MD%d Exception share memory\n",
		     md_id + 1);
	ccci_mem_dump((int *)md_ctlb[md_id]->
		      smem_table->ccci_exp_smem_base_virt,
		      md_ctlb[md_id]->smem_table->ccci_exp_smem_size);
	...
	ccci_mem_dump((int *)md_ctlb[md_id]->md_layout->md_region_vir,
		      MD_IMG_DUMP_SIZE);
	/*  Dump MD image memory */
	CCCI_DBG_MSG(md_id, "cci", "\n\n");
	...
	ccci_aed(md_id, CCCI_AED_DUMP_EX_MEM | CCCI_AED_DUMP_MD_IMG_MEM,
		 ex_info);
}


#ifdef ENABLE_DUMP_MD_REG
int ccci_dump_md_register_ext(int md_id, char buf[], unsigned int len)
{
	/* Dump Exception share memory */
	CCCI_DBG_MSG(md_id, "cci", "\n\n");
	CCCI_DBG_MSG(md_id, "cci", "Dump MD%d Exception share memory+\n", md_id + 1);
	if (!md_ctlb[md_id])
		CCCI_ERR("Dump MD%d Exception share memory fail: md_ctlb[%d] is NULL!\n", md_id + 1, md_id + 1);
	else
		ccci_mem_dump((int *)md_ctlb[md_id]->smem_table->ccci_exp_smem_base_virt
			, md_ctlb[md_id]->smem_table->ccci_exp_smem_size);
	CCCI_DBG_MSG(md_id, "cci", "Dump MD%d Exception share memory-\n", md_id + 1);
...
void config_misc_info(int md_id, unsigned int base[], unsigned int size)
{
	...
	if (NULL == base)
		return;
	memset(&misc_info, 0, sizeof(struct misc_info_t));
	...
	if (crystal_exist_status()) {
		...
		CCCI_MSG_INF(md_id, "ctl", "MISC_32K_LESS support\n");
		misc_info.support_mask |=
		    (FEATURE_SUPPORT << (MISC_32K_LESS * 2));
	}
#else
	CCCI_MSG_INF(md_id, "ctl", "ENABLE_32K_CLK_LESS disabled\n");
	misc_info.support_mask |=
		(FEATURE_NOT_SUPPORT << (MISC_32K_LESS * 2));
#endif
#ifdef ENABLE_RANDOM_SEED
	/* --- Feature 2, random seed  */
	get_random_bytes(&random_seed, sizeof(int));
	misc_info.feature_2_val[0] = random_seed;
	CCCI_MSG_INF(md_id, "ctl",
		"random_seed=%x,misc_info.feature_2_val[0]=%x,d=%d\n",
		random_seed, misc_info.feature_2_val[0], random_seed);
	misc_info.support_mask |=
		(FEATURE_SUPPORT << (MISC_RAND_SEED * 2));
#else
	misc_info.support_mask |=
		(FEATURE_NOT_SUPPORT << (MISC_RAND_SEED * 2));
#endif  /*  ENABLE_RANDOM_SEED */

	/* --- Feature 3, GPS&MD co-clk setting */
#ifdef ENABLE_GPS_MD_COCLK
	misc_info.feature_3_val[0] = wmt_get_coclock_setting_for_ccci();
	misc_info.support_mask |=
		(FEATURE_SUPPORT << (MISC_MD_COCLK_SETTING * 2));
	CCCI_MSG_INF(md_id, "ctl", "MISC_MD_COCLK_SETTING(%x)\n",
		misc_info.feature_3_val[0]);
#else
	misc_info.support_mask |=
...
static void ccci_dump_runtime_data(int md_id, struct modem_runtime_t *runtime,
				   struct smem_alloc_t *smem)
{
	int i;
	char ctmp[12];
	int *p;
	...
	*p = runtime->Platform_H;
	...
	for (i = 0; i < CCCI_UART_PORT_NUM; i++) {
		CCCI_DBG_MSG(md_id, "ctl",
			     "UartShareMemBase[%d]          0x%x\n", i,
			     runtime->UartShareMemBase[i]);
		CCCI_DBG_MSG(md_id, "ctl",
			     "UartShareMemSize[%d]          0x%x\n", i,
			     runtime->UartShareMemSize[i]);
	}
	...
	for (i = 0; i < NET_PORT_NUM; i++) {
		CCCI_DBG_MSG(md_id, "ctl",
			     "NetDLCtrlShareMemBase[%d]    0x%x\n", i,
			     runtime->NetDLCtrlShareMemBase[i]);
		CCCI_DBG_MSG(md_id, "ctl",
			     "NetDLCtrlShareMemSize[%d]    0x%x\n", i,
			     runtime->NetDLCtrlShareMemSize[i]);

		CCCI_DBG_MSG(md_id, "ctl",
			     "NetULCtrlShareMemBase[%d]    0x%x\n", i,
			     runtime->NetULCtrlShareMemBase[i]);
		CCCI_DBG_MSG(md_id, "ctl",
			     "NetULCtrlShareMemSize[%d]    0x%x\n", i,
			     runtime->NetULCtrlShareMemSize[i]);
	}
#endif
	...
	*p = runtime->Postfix;
	...
	CCCI_DBG_MSG(md_id, "ctl",
		     "md_runtime_data_smem_phy    AP=0x%x,MD=0x%x\n",
...
/*                                                                          */
/****************************************************************************/
static void ccci_md_exception(int md_id, struct DEBUG_INFO_T *debug_info)
{
	...
	ex_info =
	    (struct EX_LOG_T *) md_ctlb[md_id]->smem_table->ccci_exp_smem_base_virt;
...
#define AED_STR_LEN        (512)
	...
	if (info_str_len > AED_STR_LEN) /*  Cut string length to AED_STR_LEN */
	...
	if (dump_flag & CCCI_AED_DUMP_EX_MEM) {
		ex_log_addr =
		    (int *)md_ctlb[md_id]->smem_table->ccci_exp_smem_base_virt;
	...
	if (dump_flag & CCCI_AED_DUMP_CCIF_REG) {
		ex_log_addr =
		    (int *)md_ctlb[md_id]->smem_table->ccci_exp_smem_base_virt;
		ex_log_len = 68 * 4;
		ccci_dump_hw_reg_val(md_id, (unsigned int *)ex_log_addr,
				     ex_log_len);
	}
#if defined(CONFIG_MTK_AEE_FEATURE) && defined(ENABLE_AEE_MD_EE)
...
void md_emi_check(int md_id, struct ccci_msg_t *buff, struct DEBUG_INFO_T *debug_info)
{
...
{
	struct md_ctl_block_t *ctl_b = md_ctlb[md_id];
	struct modem_exception_exp_t *exp_ee_info;

	*p_ee_info = 0;
	exp_ee_info =
	    (struct modem_exception_exp_t *) ctl_b->
	    smem_table->ccci_md_ex_exp_info_smem_base_virt;
...
void ccci_check_md_no_physical_channel(int md_id, unsigned int args)
{
	...
	if (ee_info) {
		if (ctl_b->ee_info_got == 0) {
			...
			exp_ee_info =
			    ...
			    smem_table->ccci_md_ex_exp_info_smem_base_virt;
			CCCI_MSG_INF(md_id, "ctl",
				     "receive MD_EX @S(%08X)(%d:%d)\n",
				     exp_ee_info->exception_occur,
				     exp_ee_info->send_time,
				     exp_ee_info->wait_time);
	...
	if (trigger_ee) {
		...
		ctl_b->ee_info_flag |=
		    ((1 << MD_EE_FLOW_START) | (1 << MD_EE_FOUND_BY_TX) |
		     ee_info);
		...
		if (trigger_ee_timer)
			mod_timer(&ctl_b->md_ex_monitor,
				  jiffies + EE_TIMER_BASE + 2);
	} else {
		if (ctl_b->ee_info_got == 0) {
			switch (args) {
			case 1:	/*  pending 50ms, dump EE buffer only */
				...
				ccci_dump_hw_reg_val(md_id, NULL, 0);
				CCCI_MSG_INF(md_id, "cci",
					     "Dump MD%d Exception share memory\n",
					     md_id + 1);
				ccci_mem_dump((int *)
					      md_ctlb
					      [md_id]->smem_table->ccci_exp_smem_base_virt,
					      md_ctlb[md_id]->
					      smem_table->ccci_exp_smem_size);
				break;

			case 2:	/* pending 1500ms, trigger EE */
				CCCI_MSG_INF(md_id, "cci", "TX 1500ms\n");
				if ((ctl_b->ee_info_flag &
				     (1 << MD_EE_FLOW_START)) == 0)
					ccci_dump_hw_reg_val(md_id, NULL, 0);

				spin_lock_irqsave(&ctl_b->ctl_lock, flags);
				if ((ctl_b->ee_info_flag &
				     ((1 << MD_EE_PENDING_TOO_LONG) |
				      (1 << MD_STATE_UPDATE))) == 0)
					trigger_ee_timer = 1;
				ctl_b->ee_info_flag |=
				    ((1 << MD_EE_FLOW_START) |
...
void ccif_isr_check(int md_id)
{
	...
	if (trigger_ee) {
		spin_lock_irqsave(&ctl_b->ctl_lock, flags);
		ctl_b->ee_info_flag |=
		    ((1 << MD_EE_FLOW_START) | (1 << MD_EE_FOUND_BY_ISR) |
		     ee_info);
		if ((ctl_b->ee_info_flag & (1 << MD_STATE_UPDATE)) == 0)
			trigger_ee_timer = 1;
		ctl_b->md_boot_stage = MD_BOOT_STAGE_EXCEPTION;
...
{
	struct MD_CALL_BACK_QUEUE *queue;
	unsigned long flag;
...
{
	struct MD_CALL_BACK_HEAD_T *head;
	unsigned long flag;
	int retry = 10;
...
int md_unregister_call_chain(int md_id, struct MD_CALL_BACK_QUEUE *queue)
{
	...
	int ret = -1;
	struct MD_CALL_BACK_HEAD_T *head;
	struct MD_CALL_BACK_QUEUE **_queue = NULL;
	int retry = 10;
...
static void md_boot_up_timeout_func(unsigned long data)
{
	...
	if (ctl_b->md_boot_stage == MD_BOOT_STAGE_0) {
		...
		CCCI_MSG_INF(md_id, "cci", "Dump MD%d Image memory\n",
			     md_id + 1);
		ccci_mem_dump((int *)md_ctlb[md_id]->md_layout->md_region_vir,
			      MD_IMG_DUMP_SIZE);
#ifdef ENABLE_CCCI_DRV_BUILDIN
		CCCI_MSG_INF(md_id, "cci", "Dump TASK_UNINTERRUPTIBLE\n");
		...
		show_state_filter(TASK_UNINTERRUPTIBLE);
#endif
		ccci_dump_hw_reg_val(md_id, NULL, 256);
...
static int set_md_runtime(int md_id,
			  struct modem_runtime_t *runtime,
			  struct modem_runtime_info_tag_t *tag)
{
	...
	ctl_b = md_ctlb[md_id];
	memset(runtime, 0, sizeof(struct modem_runtime_t));
	...
	if (ctl_b->smem_table->ccci_sys_smem_size) {
		runtime->SysShareMemBase =
		    ctl_b->smem_table->ccci_sys_smem_base_phy -
		    get_md2_ap_phy_addr_fixed();
		runtime->SysShareMemSize =
		    ctl_b->smem_table->ccci_sys_smem_size;
	} else {
	...
	if (ctl_b->smem_table->ccci_exp_smem_size) {
		runtime->ExceShareMemBase =
		    ctl_b->smem_table->ccci_exp_smem_base_phy -
		    get_md2_ap_phy_addr_fixed();
		runtime->ExceShareMemSize =
		    ctl_b->smem_table->ccci_exp_smem_size;
	} else {
	...
	if (ctl_b->smem_table->ccci_mdlog_smem_size) {
		runtime->MdlogShareMemBase =
		    ctl_b->smem_table->ccci_mdlog_smem_base_phy -
		    get_md2_ap_phy_addr_fixed();
		runtime->MdlogShareMemSize =
		    ctl_b->smem_table->ccci_mdlog_smem_size;
	} else {
	...
	if (ctl_b->smem_table->ccci_pcm_smem_size) {
		runtime->PcmShareMemBase =
		    ctl_b->smem_table->ccci_pcm_smem_base_phy -
		    get_md2_ap_phy_addr_fixed();
		runtime->PcmShareMemSize =
		    ctl_b->smem_table->ccci_pcm_smem_size;
	} else {
	...
	if (ctl_b->smem_table->ccci_pmic_smem_size) {
		runtime->PmicShareMemBase =
		    ctl_b->smem_table->ccci_pmic_smem_base_phy -
		    get_md2_ap_phy_addr_fixed();
		runtime->PmicShareMemSize =
		    ctl_b->smem_table->ccci_pmic_smem_size;
	...
	if (ctl_b->smem_table->ccci_fs_smem_size) {
		runtime->FileShareMemBase =
		    ctl_b->smem_table->ccci_fs_smem_base_phy -
		    get_md2_ap_phy_addr_fixed();
		runtime->FileShareMemSize =
		    ctl_b->smem_table->ccci_fs_smem_size;
	} else {
	...
	if (ctl_b->smem_table->ccci_rpc_smem_size) {
		runtime->RpcShareMemBase =
		    ctl_b->smem_table->ccci_rpc_smem_base_phy -
		    get_md2_ap_phy_addr_fixed();
		runtime->RpcShareMemSize =
		    ctl_b->smem_table->ccci_rpc_smem_size;
	} else {
	...
	if (sizeof(CCCI_IPC_BUFFER)) {
		/*  Note this */
		runtime->IPCShareMemBase = ctl_b->smem_table->ccci_ipc_smem_base_phy
				+ offset_of(CCCI_IPC_MEM, buffer) - get_md2_ap_phy_addr_fixed();
		runtime->IPCShareMemSize = sizeof(CCCI_IPC_BUFFER);
	} else {
	...
	if (sizeof(ipc_ilm_t) * MAX_NUM_IPC_TASKS_MD) {
		runtime->IPCMDIlmShareMemBase =
		    (int)ctl_b->smem_table->ccci_ipc_smem_base_phy +
		    offset_of(CCCI_IPC_MEM,
			      ilm_md) - get_md2_ap_phy_addr_fixed();
		runtime->IPCMDIlmShareMemSize =
		    sizeof(ipc_ilm_t) * MAX_NUM_IPC_TASKS_MD;
	...
	for (i = 0; i < CCCI_UART_PORT_NUM; i++) {
		if ((ctl_b->smem_table->ccci_uart_smem_base_phy[i] != 0) &&
			...
			if (ctl_b->smem_table->ccci_uart_smem_size[i]) {
				runtime->UartShareMemBase[i] =
				    ctl_b->
				    smem_table->ccci_uart_smem_base_phy[i] -
				    get_md2_ap_phy_addr_fixed();
				runtime->UartShareMemSize[i] =
				    ctl_b->smem_table->ccci_uart_smem_size[i];
			} else {
	...
	for (i = 0; i < NET_PORT_NUM; i++) {
		...
		if (ccci_get_sub_module_cfg
		    (md_id, "net_dl_ctl", (char *)&dl_ctl_mem_size,
		...
		if (dl_ctl_mem_size) {
			runtime->NetDLCtrlShareMemBase[i] =
			    ctl_b->smem_table->ccci_ccmni_ctl_smem_base_phy[i] -
			    get_md2_ap_phy_addr_fixed();
		...
		if (ul_ctl_mem_size) {
			runtime->NetULCtrlShareMemBase[i] =
			    ctl_b->smem_table->ccci_ccmni_ctl_smem_base_phy[i] +
			    dl_ctl_mem_size - get_md2_ap_phy_addr_fixed();
	...
	if (ctl_b->smem_table->ccci_ccmni_smem_dl_size) {
		runtime->MDDLNetShareMemBase =
		    ctl_b->smem_table->ccci_ccmni_smem_dl_base_phy -
		    get_md2_ap_phy_addr_fixed();
		runtime->MDDLNetShareMemSize =
		    ctl_b->smem_table->ccci_ccmni_smem_dl_size;
	} else {
	...
	if (ctl_b->smem_table->ccci_ccmni_smem_ul_size) {
		runtime->MDULNetShareMemBase =
		    ctl_b->smem_table->ccci_ccmni_smem_ul_base_phy -
		    get_md2_ap_phy_addr_fixed();
		runtime->MDULNetShareMemSize =
		    ctl_b->smem_table->ccci_ccmni_smem_ul_size;
	} else {
	...
	if (ctl_b->smem_table->ccci_md_ex_exp_info_smem_size) {
		runtime->MDExExpInfoBase =
		    ctl_b->smem_table->ccci_md_ex_exp_info_smem_base_phy -
		    get_md2_ap_phy_addr_fixed();
		runtime->MDExExpInfoSize =
		    ctl_b->smem_table->ccci_md_ex_exp_info_smem_size;
	} else {
	...
	if (is_meta_mode() || is_advanced_meta_mode())
		runtime->BootingStartID =
		    ((char)mdlog_flag << 8 | META_BOOT_ID);
	else
		runtime->BootingStartID =
		    ((char)mdlog_flag << 8 | NORMAL_BOOT_ID);
	...
	config_misc_info(md_id,
			 (unsigned int *)ctl_b->
			 smem_table->ccci_misc_info_base_virt,
			 ctl_b->smem_table->ccci_misc_info_size);
	...
	tag->runtime_data_base =
	    ctl_b->smem_table->ccci_md_runtime_data_smem_base_phy -
	    get_md2_ap_phy_addr_fixed();
	tag->runtime_data_size = sizeof(struct modem_runtime_t);
	CCCI_MSG_INF(md_id, "ctl", "set runtime data: size=%d!\n",
		     sizeof(struct modem_runtime_t));
...
int ccci_send_run_time_data(int md_id)
{
	int ret = 0;
	struct md_ctl_block_t *ctl_b;
	struct modem_runtime_info_tag_t runtime_tag;
	struct ccci_msg_t msg;
	struct modem_runtime_t *runtime;
	...
	runtime =
	    (struct modem_runtime_t *) (ctl_b->
				 smem_table->ccci_md_runtime_data_smem_base_virt);
	...
	ret =
	    ccci_write_runtime_data(md_id, (unsigned char *)&runtime_tag,
				    sizeof(struct modem_runtime_info_tag_t));
	if (ret < 0) {
		CCCI_MSG_INF(md_id, "ctl",
			     "fail to write MODEM runtime data(%d)\n", ret);
		return ret;
	}
	...
	mb();/* waiting mem updated*/

	ret = ccci_message_send(md_id, &msg, 1);
	if (ret != sizeof(msg)) {
		CCCI_MSG_INF(md_id, "ctl",
			     "fail to write CCCI_CONTROL_TX(%d)\n", ret);
		return ret;
	}
...
int ccci_reset_register(int md_id, char *name)
{
	...
	if (name == NULL) {
		CCCI_MSG_INF(md_id, "ctl",
			     "[Error]Invalid reset handle name\n");
		return -CCCI_ERR_INVALID_PARAM;
	}
	...
	if (handle < NR_CCCI_RESET_USER) {
		...
		CCCI_MSG_INF(md_id, "ctl",
			     "Register a reset handle by %s(%d)\n",
			     current->comm, handle);
		return handle;
	}
	mutex_unlock(&ctl_b->ccci_reset_mutex);
...
int ccci_user_ready_to_reset(int md_id, int handle)
{
	...
	if (handle >= NR_CCCI_RESET_USER) {
		CCCI_MSG_INF(md_id, "ctl",
			"reset_request: invalid handle:%d\n", handle);
		return -CCCI_ERR_INVALID_PARAM;
	}

	if (ctl_b->reset_sta[handle].is_allocate == 0) {
		CCCI_MSG_INF(md_id, "ctl",
			     "reset_request: handle(%d) not alloc: alloc=%d\n",
	...
	for (i = 0; i < NR_CCCI_RESET_USER; i++) {
		if (ctl_b->reset_sta[i].is_allocate
		    && (ctl_b->reset_sta[i].is_reset == 0)) {
			reset_ready = 0;
			CCCI_MSG_INF(md_id, "ctl", " ==> %s\n",
...
void ccci_md_ctrl_cb(void *private)
{
	...
	struct ccci_msg_t msg;
	int ret;
	struct md_ctl_block_t *ctl_b = (struct md_ctl_block_t *) ch_info->m_owner;
	int md_id = ctl_b->m_md_id;
	struct DEBUG_INFO_T debug_info;
	unsigned long flags;
	int need_update_state = 0;
	...
	while (get_logic_ch_data(ch_info, &msg)) {
		if (msg.id == MD_INIT_START_BOOT &&
			...
			CCCI_MSG_INF(md_id, "ctl",
				     "AP CCCI driver version mis-match to MD!!\n");
			ctl_b->stop_retry_boot_md = 1;
			ccci_aed(md_id, 0, "AP/MD driver version mis-match\n");
		} else {
			CCCI_MSG_INF(md_id, "ctl",
				     "receive unknown data from CCCI_CONTROL_RX = %d\n",
				     msg.id);
		}
	}
}
...
static int boot_md(int md_id)
{
	...
	CCCI_MSG_INF(md_id, "ctl",
		     "booting up MODEM: start to load firmware...\n");
...
int ccci_ipo_h_restore(int md_id, char buf[], unsigned int len)
{
	...
	CCCI_DBG_MSG(md_id, "ctl", "IPO-H: SMEM add:0x%08x, size:%d\n",
		     ctl_b->smem_table->ccci_smem_vir,
		     ctl_b->smem_table->ccci_smem_size);
	memset((void *)ctl_b->smem_table->ccci_smem_vir, 0,
	       ctl_b->smem_table->ccci_smem_size);
	/*  2-1 UART */
...
{
	int ret;
	struct md_ctl_block_t *ctlb;

	/*  Allocate md ctrl struct memory */
	ctlb = kmalloc(sizeof(struct md_ctl_block_t), GFP_KERNEL);
	if (ctlb == NULL)
		return -CCCI_ERR_GET_MEM_FAIL;

	/*  Init control struct */
	memset(ctlb, 0, sizeof(struct md_ctl_block_t));
	...
	tasklet_init(&ctlb->md_notifier.tasklet, notify_chain,
		     (unsigned long)ctlb);
	...
	register_ccci_kern_func_by_md_id(md_id, ID_IPO_H_RESTORE_CB,
					 ccci_ipo_h_restore);

	register_ccci_kern_func_by_md_id(md_id, ID_FORCE_MD_ASSERT,
					 ccci_force_md_assert);

#ifdef ENABLE_DUMP_MD_REG
	register_ccci_kern_func_by_md_id(md_id, ID_DUMP_MD_REG, ccci_dump_md_register_ext);
#endif /* ENABLE_DUMP_MD_REG */
	/* Clear all share memory to zero */
	memset((void *)ctlb->smem_table->ccci_smem_vir, 0,
	       ctlb->smem_table->ccci_smem_size);
...
{
	struct md_ctl_block_t *ctlb = md_ctlb[md_id];
