...
extern const char* get_cci_project_id_name(void);
extern int SENSOR_OPTION;
int hwid=-1;
#define AP_build_hwid 4

#define TP_build_hwid 5
...
#ifdef STK_TUNE0
	#define STK_MAX_MIN_DIFF	600
	#define STK_LT_N_CT	600
	#define STK_HT_N_CT	900
...
{
	hwid=board_type_with_hw_id();//get_cci_hw_id();
	APS_LOG("%s: platform version=%d\n", __FUNCTION__, hwid);
...
/**/
	if(AP_build_hwid==board_type_with_hw_id() || TP_build_hwid==board_type_with_hw_id() ){ 
		...
		if((cci_ps_high_thd-cci_ps_low_thd)<150){
			if((cci_ps_low_thd - cci_result_ct)>400){
				offset_H=0;
				offset_L=200;
				}
			else{
			...
			offset_H=cci_ps_high_thd -cci_ps_low_thd;
			offset_L=(int)((cci_ps_low_thd - cci_result_ct)/2);
		}
	}
	else{
		offset_H=0;
...
static ssize_t stk3x1x_show_nvdata(struct device_driver *ddri, char *buf)
{
	...
	APS_ERR("[STK]offset_H =  %d, offset_L =  %d\n", offset_H,offset_L);
	return scnprintf(buf, PAGE_SIZE, "cci_ps_high_thd = %d, cci_ps_low_thd = %d, cci_result_ct = %d, cci_ps_cover = %d, cci_als_value = %d, cci_als_adc_test = %d, ,cci_als_lux_cali=%d , cci_transmittance_cali = %d, cci_als_adc_cali =%d, offset_H=%d, offset_L=%d\n", cci_ps_high_thd, cci_ps_low_thd, cci_result_ct, cci_ps_cover, cci_als_value,cci_als_adc_test,cci_als_lux_cali, cci_transmittance_cali, cci_als_adc_cali,offset_H,offset_L );  		
}
...
static ssize_t stk3x1x_show_GC_20mm(struct device_driver *ddri, char *buf)
{
	int res=0;
	uint32_t reading=0;
	int err=0;//,count=0;
	...
	//set default THD as ps_thd_h = 300, ps_thd_l = 100 to ensure int trigger start
	...
	if((err = stk3x1x_write_ps_low_thd(stk3x1x_obj->client, atomic_read(&stk3x1x_obj->ps_low_thd_val))))
	{
		...
		return scnprintf(buf, PAGE_SIZE, "write low thd error: %d\n", err);
	}	
	...
	if((err = stk3x1x_write_ps_high_thd(stk3x1x_obj->client, atomic_read(&stk3x1x_obj->ps_high_thd_val))))
	{
		...
		return scnprintf(buf, PAGE_SIZE, "write high thd error: %d\n", err);
	}	

	//set default THD as ps_thd_h = 300, ps_thd_l = 100 to ensure int trigger end
	APS_ERR("[STK]%s:Start calibrating High threshold...\n", __func__);
	msleep(150);
	reading = stk3x1x_get_ps_reading_AVG(5);

	cci_ps_high_thd = reading; //*****************This is F2N threshold***************
	APS_ERR("[STK]%s:calibrating High threshold done!!!!! cci_ps_high_thd = %d\n", __func__, cci_ps_high_thd);
	return scnprintf(buf, PAGE_SIZE, "cci_ps_high_thd = %d\n", cci_ps_high_thd);
}
static ssize_t stk3x1x_show_GC_30mm(struct device_driver *ddri, char *buf)
{
	uint32_t reading=0;
	//int err,count=0;
	if(!stk3x1x_obj)
	...
	APS_ERR("[STK]%s:Start calibrating Low threshold..\n", __func__);
	msleep(150);
	reading = stk3x1x_get_ps_reading_AVG(5);

	cci_ps_low_thd = reading; //*****************This is N2F threshold***************
	APS_ERR("[STK]%s:calibrating Low threshold done. cci_ps_low_thd = %d\n", __func__, cci_ps_low_thd);
	return scnprintf(buf, PAGE_SIZE, "cci_ps_low_thd = %d\n", cci_ps_low_thd);
}

static ssize_t stk3x1x_show_GC_40mm(struct device_driver *ddri, char *buf)
{
	APS_LOG("%s: Calibrate High threshold 4cm, this platform version=%d\n", __FUNCTION__, hwid);
	stk3x1x_show_GC_20mm(ddri,buf);
	return scnprintf(buf, PAGE_SIZE, "cci_ps_high_thd = %d\n", cci_ps_high_thd);
}
static ssize_t stk3x1x_show_GC_50mm(struct device_driver *ddri, char *buf)
{
	APS_LOG("%s: Calibrate Low threshold 5cm, this platform version=%d\n", __FUNCTION__, hwid);
	stk3x1x_show_GC_30mm(ddri, buf);
	return scnprintf(buf, PAGE_SIZE, "cci_ps_low_thd = %d\n", cci_ps_low_thd);
}
static ssize_t stk3x1x_show_GC_ct(struct device_driver *ddri, char *buf)
{
	...
	uint32_t reading=0;
	int Diff_ThdL_CT=0,Diff_ThdH_L=0;
	int err=0;//,count=0,low_thd_temp;
	bool result = false;
	//get ps code for 5 times
	APS_ERR("[STK]%s:Start calibrating CT...\n", __func__);
	reading = stk3x1x_get_ps_reading_AVG(5);
	...
	if (cci_result_ct > 0) {
		...
		if(hwid>=AP_build_hwid){
		if((Diff_ThdL_CT > 200)&&(Diff_ThdH_L>150)&&(cci_result_ct < 1700)){
			APS_ERR("[#23]%s: ps_CCI_cali_BC_ct_show() cci_ps_high_thd = %d, cci_ps_low_thd = %d\n", __FUNCTION__, cci_ps_high_thd, cci_ps_low_thd);
			...
			{
				APS_ERR("[STK]write high thd error: %d\n", err);
				//return scnprintf(buf, PAGE_SIZE, "FAIL:write high thd error: %d\n", err);
			}
			if((err = stk3x1x_write_ps_low_thd(stk3x1x_obj->client, atomic_read(&stk3x1x_obj->ps_low_thd_val))))
			{
				APS_ERR("[STK]write low thd error: %d\n", err);
				//return scnprintf(buf, PAGE_SIZE, "FAIL:write low thd error: %d\n", err);
			}
			result = true;
			APS_ERR("[STK]---------------Test PS Gray card CT successfully----------------\n");
			APS_ERR("[STK]cci_ps_low_thd = %d\n", cci_ps_low_thd);
		} else {

			APS_ERR("[STK]---------------Test PS Gray card CT fail----------------\n");
			APS_ERR("[STK]cci_ps_high_thd = %d cci_ps_low_thd = %d cci_result_ct = %d \n",cci_ps_high_thd, cci_ps_low_thd,cci_result_ct);
			//return scnprintf(buf, PAGE_SIZE, "FAIL:Out of range, cci_result_ct = %d\n",cci_result_ct);
		}
		}
		else{
			if((Diff_ThdL_CT > 200)&&(cci_ps_high_thd > cci_ps_low_thd)&&(cci_result_ct < 10000)){
			APS_ERR("[#23]%s: ps_CCI_cali_BC_ct_show() cci_ps_high_thd = %d, cci_ps_low_thd = %d\n", __FUNCTION__, cci_ps_high_thd, cci_ps_low_thd);
		...
		APS_ERR("[STK]---------------Test PS Gray card CT fail----------------\n");
		//return scnprintf(buf, PAGE_SIZE, "FAIL:Out of range, cci_ps_high_thd = %d, cci_ps_low_thd=%d,  cci_result_ct = %d\n",cci_ps_high_thd, cci_ps_low_thd,cci_result_ct);
	}

	printk(KERN_ERR "[STK]%s : cci_result_ct = %d, Diff_ThdL_CT = %d, cci_ps_high_thd = %d\n", result ? "PASS" : "FAIL",cci_result_ct, Diff_ThdL_CT, cci_ps_high_thd);
	return scnprintf(buf, PAGE_SIZE, "%s: cci_result_ct = %d, cci_ps_high_thd = %d \n",  result ? "PASS" : "FAIL", cci_result_ct, cci_ps_high_thd);
}

static ssize_t stk3x1x_show_test_GC(struct device_driver *ddri, char *buf)
{
	//Test black card cover
	uint32_t reading=0;
	uint32_t test_ps_code=0;
	bool ps_test_cover=0;
	//int err,count=0;
	
	//get ps code for 5 times
	APS_ERR("%s:[#23] Start testing GC...\n", __func__);
	reading = stk3x1x_get_ps_reading_AVG(5);
	
	test_ps_code = reading;
	
	APS_ERR("%s:[STK]ps_CCI_test_GC_show() CCI_FTM_INTERRUPT = %d\n", __func__, CCI_FTM_INTERRUPT);
	APS_ERR("[STK] adc= %d\n",test_ps_code);
	if(test_ps_code >= cci_ps_high_thd){
		ps_test_cover = true;
		cci_ps_cover = test_ps_code;
		if(CCI_FTM_INTERRUPT != 0){
			APS_ERR("[#23]SUCCESS: ps_test_cover = %d, g_result_ps_cover = %d, INT PASS\n", ps_test_cover, cci_ps_cover);
			return scnprintf(buf, PAGE_SIZE, "SUCCESS: ps_test_cover = %d, g_result_ps_cover = %d, INT PASS\n", ps_test_cover, cci_ps_cover);
		} else {
			ps_test_cover = false;
			APS_ERR("[#23]FAIL: ps_test_cover = %d, g_result_ps_cover = %d, INT FAIL\n", ps_test_cover, cci_ps_cover);
			return scnprintf(buf, PAGE_SIZE, "FAIL: ps_test_cover = %d, g_result_ps_cover = %d, INT FAIL\n", ps_test_cover, cci_ps_cover);
		}
	} else {
		ps_test_cover = false;
		cci_ps_cover = test_ps_code;
		...
		if(CCI_FTM_INTERRUPT != 0){
			APS_ERR("[#23]FAIL: ps_test_cover = %d, g_result_ps_cover = %d, INT PASS\n", ps_test_cover, cci_ps_cover);
			return scnprintf(buf, PAGE_SIZE, "FAIL: ps_test_cover = %d, g_result_ps_cover = %d, INT PASS\n", ps_test_cover, cci_ps_cover);
		} else {
			APS_ERR("[#23]FAIL: ps_test_cover = %d, g_result_ps_cover = %d, INT FAIL\n", ps_test_cover, cci_ps_cover);
			return scnprintf(buf, PAGE_SIZE, "FAIL: ps_test_cover = %d, g_result_ps_cover = %d, INT FAIL\n", ps_test_cover, cci_ps_cover);
		}
...
static ssize_t stk3x1x_show_test_Light(struct device_driver *ddri, char *buf)
{
	//Test ALS 
	//int32_t als_reading;
	//int count=0;
	APS_ERR("[#23]%s: Start testing light...\n", __func__);
	msleep(150);
	cci_als_adc_test = stk3x1x_get_als_reading_AVG(5);

	APS_ERR("[#23]%s: als_reading = %d\n", __func__,cci_als_adc_test);
	cci_als_value = stk_alscode2lux(cci_als_adc_test);

	//cci_als_value = als_reading; //return raw data only
	
	APS_ERR("[#23]%s: Start testing light done!!! cci_als_value = %d lux\n", __func__, cci_als_value);
	if(cci_als_value >= 270 && cci_als_value <= 330)
		return scnprintf(buf, PAGE_SIZE, "PASS. cci_als_value = %d, cci_als_adc_test = %d \n", cci_als_value,cci_als_adc_test);
	else
		return scnprintf(buf, PAGE_SIZE, "FAIL. cci_als_value = %d, cci_als_adc_test = %d \n", cci_als_value,cci_als_adc_test);
}

static ssize_t als_CCI_cali_Light_show(struct device_driver *ddri, char *buf)
{
    int ret=0;//,cal_als_adc_tmp,cal_transmittance_tmp,count=0;
    bool result=false;
    APS_LOG("%s, enter\n", __func__);
    ret = stk3x1x_cci_als_cali(5);
	cci_als_lux_cali = stk_alscode2lux(cci_als_adc_cali);

	if( ret && cci_als_adc_cali >= 94 && cci_als_adc_cali <= 962 ){
			printk(KERN_ERR "%s:[#23][STK]cali light done!!! cci_als_value_cali = %d lux, cci_transmittance_cali = %d, cci_als_value_cali_adc = %d code\n", __func__, cci_als_lux_cali, cci_transmittance_cali, cci_als_adc_cali);
			result = true;
	}
	else{
...
static ssize_t stk3x1x_show_projectID(struct device_driver *ddri, char *buf)
{
	if (strncmp(get_cci_project_id_name(), "VY", 2) == 0)
	{
		printk("project ID = %s \n",get_cci_project_id_name());
		 return scnprintf(buf, PAGE_SIZE,"%s\n","1");
	}
	else {
		printk("Error, project ID = %s \n",get_cci_project_id_name());
		return scnprintf(buf, PAGE_SIZE,"%s\n","0");
	}
}
...
static DRIVER_ATTR(GC_20mm,    S_IWUSR | S_IRUGO, stk3x1x_show_GC_20mm,  NULL);
static DRIVER_ATTR(GC_30mm,    S_IWUSR | S_IRUGO, stk3x1x_show_GC_30mm,  NULL);
static DRIVER_ATTR(GC_40mm,    S_IWUSR | S_IRUGO, stk3x1x_show_GC_40mm,  NULL);
static DRIVER_ATTR(GC_50mm,    S_IWUSR | S_IRUGO, stk3x1x_show_GC_50mm,  NULL);
static DRIVER_ATTR(GC_ct_150mm,    S_IWUSR | S_IRUGO, stk3x1x_show_GC_ct,  NULL);
static DRIVER_ATTR(test_GC_10mm,    S_IWUSR | S_IRUGO, stk3x1x_show_test_GC,  NULL);
static DRIVER_ATTR(test_Light,    S_IWUSR | S_IRUGO, stk3x1x_show_test_Light,  NULL);
...
static DRIVER_ATTR(projectID,S_IWUSR | S_IRUGO,stk3x1x_show_projectID,NULL);
...
//    &driver_attr_i2c,
    ...
    &driver_attr_GC_ct_150mm,
    &driver_attr_test_GC_10mm,
