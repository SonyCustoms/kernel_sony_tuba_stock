...
//CEI comment start//
	if (g_fg_is_charging == KAL_TRUE){
		bm_print(BM_LOG_CRTI, "[fgauge_read_current] current(charging) = %d mA\r\n",
			 dvalue);
		temp_is_charging =KAL_TRUE;
	}
	else{
		bm_print(BM_LOG_CRTI, "[fgauge_read_current] current(discharging) = %d mA\r\n",
			 dvalue);
		temp_is_charging =KAL_FALSE;
	}
//CEI comment end//
...
//CEI comment start//
		bm_print(BM_LOG_CRTI, "[fgauge_read_current] final current=%d (ratio=%d) => %s%d mA (%s)\n",
			 dvalue, batt_meter_cust_data.car_tune_value, temp_is_charging ? "-":"+", dvalue/10, temp_is_charging ? "charging":"discharging");
//CEI comment end//
...
//CEI comment start//
		bm_print(BM_LOG_CRTI, "[fgauge_read_IM_current] final current=%d (ratio=%d) => %s%d mA (%s)\n",
			 dvalue, batt_meter_cust_data.car_tune_value, temp_is_charging ? "-":"+", dvalue/10, temp_is_charging ? "charging":"discharging");
//CEI comment end//
...
//CEI comment start//
	bm_print(BM_LOG_CRTI, "[fgauge_read_columb_internal] final dvalue_CAR = %d\r\n",
		 dvalue_CAR);
//CEI comment end//
...
//CEI comments start//
extern unsigned int PMIC_IMM_GetOneChannelValueMD(unsigned char dwChannel, int deCount, int trimd);

static signed int read_adc_v_bat_id(void *data)
{
	unsigned int adc_level = 0;

	adc_level = PMIC_IMM_GetOneChannelValueMD(0x7,*(signed int*)(data),1);
	*(signed int*)(data)  =  (adc_level * 1800)/32768;
